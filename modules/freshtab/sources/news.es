  ...
  return new Promise(function (resolve, reject)  {
    ...
    CliqzHistoryManager.PlacesInterestsStorage._execute(
      ...
      function(result){
        ...
        if (c)
          // take visit count only for the exact domain
          if ((url_desc.path === '/')||(url_desc.path.indexOf('/index') === 0)){
...
function getNewsDistributionUrlBased(topic_db, records_number){
  ...
  function subDomainCount(subDomain, rn, pathList){
    ...
    var newsAddFlag = true;
    // in case of not all fields are filled on the top level, put the most frequent domain
    if ((pathList.length === 0)&&((rn - added_at_level)>0)){
      for (var k in results_list){
        if ((results_list[k][0].length === 1)&&(subDomainList[0] && results_list[k][0][0] === subDomainList[0].key)){
          results_list[k][1] += rn - added_at_level;
          newsAddFlag = false;
        }
      }
      if (newsAddFlag && subDomainList[0]){
        results_list.push([[subDomainList[0].key], rn - added_at_level])
      }
    }
