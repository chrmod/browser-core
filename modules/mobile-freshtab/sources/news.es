...
function displayTopSites (list, isEditMode) {

  var blockedTopSites = CLIQZEnvironment.getLocalStorage().getList('blockedTopSites');
  ...
  div.innerHTML = topSites({isEditMode, list});


  CLIQZEnvironment.addEventListenerToElements('#cancelEditTopsites', 'click', function () {
    var elements = document.querySelectorAll('.blockTopsite');
  ...
  CLIQZEnvironment.addEventListenerToElements('.blockTopsite', 'click', function () {
    var blockedTopSites = CLIQZEnvironment.getLocalStorage().getList('blockedTopSites');
    blockedTopSites.push(this.getAttribute('url'));
  ...
  function longpress (element) {
    ...
    document.getElementById('cancelEditTopsites').style.display = 'block';
  }
  ...
  new LongPress('.topSitesLink', longpress, tap);
...
var News = {
  ...
  startPageHandler: function (list) {
    ...
    if(list.length < 4) {
      ...
      mockedTopSites.forEach(topsite => {
        for(let i = 0; i < size; i++) {
          if(topsite.mainDomain === list[i].mainDomain) {
            return;
          }
        }
        list.push(topsite);
      });
      // list = list.concat(mockedTopSites);
    }

    topSitesList = list;
    displayTopSites(list, false);
  },
