(function(win){
  ...
  CliqzUtils.getDomain = function(hostname){
  ...
  CliqzUtils.getDomainFromUrl = function(url){
    var noPathUrl = CliqzUtils.cleanUrl(url).split('/')[0];
    return CliqzUtils.getDomain(noPathUrl);
  }

  CliqzUtils.getIconText = function(domain){
  ...
  $.getJSON('media/js/colors.json', function(data){
    BRAND_COLORS = data;
  });

  CliqzUtils.geticonColor = function(url){
    var noPathUrl = CliqzUtils.cleanUrl(url).split('/')[0],
        signature = 0,
        brand = CliqzUtils.getDomain(noPathUrl), // only 'brand' eg: google
  ...
  CliqzUtils.cleanUrl = function(url){
    url = url.split('://')[1];
  ...
  CliqzUtils.createIconFromUrl = function($div, url){
    $div[0].style.backgroundColor = CliqzUtils.geticonColor(url);

    var domain = CliqzUtils.getDomainFromUrl(url).toLowerCase();
    if (LOGOS.indexOf(domain) != -1){
      $div[0].style.backgroundImage = 'url(media/images/logos/' + domain + '.svg)';
    } else {
      $div.text(CliqzUtils.getIconText(domain));
    }
  }

  CliqzUtils.emphasis = function(text, q){
    if(!text) return '';
