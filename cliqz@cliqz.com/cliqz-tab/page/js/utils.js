(function(win){
  ...
  SearchUtils.getDomain = function(hostname){
    var v = hostname.toLowerCase().split('.');
    ...
    // remove the nulls
    v.filter(function(n){ return n != null })

    return v[v.length-1];
  }

  SearchUtils.getDomainFromUrl = function(url){
    var noPathUrl = SearchUtils.cleanUrl(url).split('/')[0];
    return SearchUtils.getDomain(noPathUrl);
  }

  SearchUtils.getIconText = function(domain){
    return domain[0].toUpperCase() + domain[1].toLowerCase();
  }
  ...
  jQuery.getJSON('js/colors.json',null,function(data){
    BRAND_COLORS = data;
  });
    
  SearchUtils.geticonColor = function(url){
    var noPathUrl = SearchUtils.cleanUrl(url).split('/')[0],
        signature = 0,
        brand = SearchUtils.getDomain(noPathUrl), // only 'brand' eg: google
        domain = brand + noPathUrl.split(brand)[1]; // domain name eg: google.com

    for(var i=0; i<noPathUrl.length; i++) signature += noPathUrl.charCodeAt(i);

    return BRAND_COLORS[domain] ||
           BRAND_COLORS[brand] ||
           COLOURS[signature%COLOURS.length];  // fallback - solid colour
  }


  SearchUtils.cleanUrl = function(url){
    url = url.split('://')[1];
  ...
  SearchUtils.createIconFromUrl = function($div, url){
    $div[0].style.backgroundColor = SearchUtils.geticonColor(url);

    var domain = SearchUtils.getDomainFromUrl(url);
    
    if (domain) {
        domain = domain.toLowerCase();
        
        if (LOGOS.indexOf(domain) != -1){
          $div[0].style.backgroundImage = 'url(images/logos/' + domain + '.svg)';
        } else {
          $div.text(SearchUtils.getIconText(domain));
        }
    }
  }

  SearchUtils.emphasis = function(text, q){
    if(!text) return '';

    var map = Array(text.length),
        tokens = q.trim().toLowerCase().split(/\s+/),
