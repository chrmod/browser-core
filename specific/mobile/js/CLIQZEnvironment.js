CLIQZEnvironment = {
  ...
  getNews: function(newsDomains) {
    ...
    if(newsDomains) {
        ...
        callback = function(data) {
            try {
                ...
                for(var i in newsDomains) {
                    ...
                    if(news[domain]) {
                      for(var j in news[domain]) {
                        if(!CLIQZEnvironment._recentHistory[news[domain][j].url]) {
                ...
                CLIQZEnvironment.collectNews('history',myNews);
            } catch(e) {
                console.error(e);
            }
        },
        ...
        timeout = function() {
          ...
          CLIQZEnvironment.getNews(newsDomains);
        },
        data = null, 
        asynchronous = true;
    ...
    callback = function(data) { 
        try {
            var sResponse = JSON.parse(data.responseText);
            CLIQZEnvironment.collectNews('standard',sResponse.results[0].articles);
        } catch(e) {
            console.error(e);
        }
    },
  ...
  collectNews: function(type,news) {
    if(type=="history") {
        CLIQZEnvironment.newsHistory = news;
    }
    if(type=="standard") {
        CLIQZEnvironment.newsStandard = news;
    }
    if(CLIQZEnvironment.newsHistory !== null && CLIQZEnvironment.newsStandard  !== null) {
       if(CLIQZEnvironment.newsHistory) {
          CLIQZEnvironment.displayTopNews(CLIQZEnvironment.newsHistory.concat(CLIQZEnvironment.newsStandard));
       } else {
         CLIQZEnvironment.displayTopNews(CLIQZEnvironment.newsStandard);
       }
    }
  },
  ...
  displayTopNews: function(top_news) {
    ...
    CLIQZEnvironment.newsHistory = CLIQZEnvironment.newsStandard = null;
    if(!top_news) {
      return;
    }
  ...
  getRecentHistory: function(history) {
    for(var i in history.results) {
         CLIQZEnvironment._recentHistory[history.results[i].url] = true;
    }
  },
  ...
  displayTopSites: function (list) {
    ...
    osBridge.searchHistory("","CLIQZEnvironment.getRecentHistory");
    ...
    CLIQZEnvironment.getNews(newsDomainList);
  ...
  initHomepage: function(firstTime) {
    ...
    osBridge.getTopSites('CLIQZEnvironment.displayTopSites', 20);
  },
