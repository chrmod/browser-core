...
var sourceMap = {
	...
	"doorwaypage" :{
		"keys":["action", "payload.url", "payload.durl"],
		"endpoint": "safe-browsing"
	},
	"linkedin" :{
		"keys":["action", "payload.profileLink","ts"],
		"endpoint": "safe-browsing"
	},
	"suspiciousUrl" :{
		"keys":["action", "payload.qurl","ts"],
		"endpoint": "safe-browsing"
	},
	"extension-query" :{
		"keys":["ts"],
...
messageContext.prototype.aesDecrypt = function(msg){
	...
	var promise = new Promise(function(resolve, reject){
		try{
			...
			var iv = _this.iv;
			CliqzUtils.log("Encrypt: "+ encryptedMsg,"XXXX234");
			CliqzUtils.log("Key: "+ key,"XXXX234");
			CliqzUtils.log("IV: "+ iv,"XXXX234");
...
messageContext.prototype.getProxyIP = function(routeHash){
	...
	// return proxyIP;
	return CliqzSecureMessage.routeTable[modRoute][0];
}
...
/**
The payload recieved for sending across to backend, has the following fields:
1. UserPublic Key, needed to verify the signature.
2. mP: Encrypted payload, that needs to be sent to the backend.
3. dmC : The route hash needed to maintain the counter.
4. BS1 : Blind signature 1
5. BS2 : Blind signature 2
6. BS3 : Blind signature 3
7. Signature : Signature of the complete message.

The following checks need to be verified:
1. The UserPK corresponds to the a legitimate user.
2. mP has not been altered, by verified BS.
3. Message passes temporal uniqueness validation.
 */
var tuMessageContext = function (msg) {
	this.status = false;
	if(!msg) return;
	var je = new JSEncrypt();
	var fields = ["uPK", "mP", "dmC", "bs1", "bs2", "bs3", "sig"];
	var messageToVerify = "";
 	this.orgMessage = isJson(msg) ? msg : JSON.parse(msg);
 	this.payload = {};
 		for(var i=0;i<fields.length;i++){
 		CliqzUtils.log(this.orgMessage[fields[i]], fields[i]);
 		this.payload[fields[i]] = this.orgMessage[fields[i]];

 	}
 	messageToVerify = this.payload["uPK"] + ";" + this.payload["mP"] + ";" + this.payload["dmC"] + ";" + this.payload["bs1"] + ";" + this.payload["bs2"] + ";" + this.payload["bs3"];


 	var rsa = new CliqzSecureMessage.RSAKey();
	rsa.setPublic(je.parseKeyValues(this.payload["uPK"])["n"], "10001");
	var hSig = rsa.verifyHexSignatureForMessage(this.payload["sig"], messageToVerify);
	CliqzUtils.log(hSig,"Signature verfied");
	if(!hSig) this.status = false;



	var mPHash = sha256_digest(this.payload["mP"]);
	var bSig = verifyBlindSignature(this.payload["bs1"], mPHash);
	CliqzUtils.log(bSig,"BS verification");
	if(!bSig) this.status = false;

	var key = this.payload["uPK"] + ";" + this.payload["dmC"];
	var val = CliqzSecureMessage.temporalUniquness[key];
	if(val){
		CliqzUtils.log("false","temporal uniqueness");
		this.status = false;
	}
	else{
		CliqzSecureMessage.temporalUniquness[key] = {"ts":1};
		this.status = true;
	}
...
var CliqzSecureMessage = {
    ...
    pacemaker: function() {
        ...
        if ((CliqzSecureMessage.counter/CliqzSecureMessage.tmult) % (60 * 20 * 1) == 0) {
            if (CliqzSecureMessage.debug) {
                CliqzUtils.log('Load ts config', CliqzSecureMessage.LOG_KEY);
            }
            fetchSourceMapping();
	...
	getQueryConnection: function(id){
		if(!id){
			if(CliqzSecureMessage.proxyList.length > 1){
				var id = CliqzSecureMessage.proxyList.splice(0,1);
			}
			else{
	...
	getConnection: function(id, meta){
		...
		/*
		CliqzSecureMessage.wsconn.on("close", function(){
			CliqzUtils.log("Peer connection disconnected","XXXXXXX");
			CliqzSecureMessage.getConnection("480");
		});

		CliqzSecureMessage.wsconn.on('error', function(err) {
			CliqzUtils.log("Error: " + err,"ERROR");
			CliqzUtils.log("Error: " + err.type,"ERROR");

			CliqzSecureMessage.wsconn.close();
			// CliqzSecureMessage.getConnection("480");
		});
		*/
...
function connect(c) {
	CliqzSecureMessage.peerStatus = true;
    c.on('data', function(data) {
    	if(c.metadata.message == "validation"){
    		var result = {};
    		var tuM = new tuMessageContext(data);
    		if(tuM.status){
		    	var _data = JSON.parse(data);
				var rsa = new CliqzSecureMessage.RSAKey();
				rsa.readPrivateKeyFromPEMString(CliqzSecureMessage.uPK.privateKey);
				var hSig = rsa.sign(_data.bs1,"sha256");
				result["status"] = true;
				result["sig"] = hSig;
				c.send(JSON.stringify(result));
    		}
    		else{
	    		CliqzUtils.log("Sorry temporalUniquness failed","proxy-Status");
				result["status"] = false;
				result["sig"] = "";
	    		c.send(JSON.stringify(result));
	    	}
    		// Validate the message, if it is validated then send to other proxies. else return from here.

    		/*
    		var requestedPeer = c.metadata.id;
    		//sendMessage(pp, data);
    		Promise.all([sendMessage(requestedPeer, "data1"), sendMessage(requestedPeer, "data2")]).then(function(values) {
 				 CliqzUtils.log(CliqzSecureMessage.resp,"RESP"); // [3, 1337, "foo"]
			});
			*/
    	}
    	else if(c.metadata.message == "query"){
    		CliqzUtils.log(data)
		    CliqzUtils.httpPost('http://securebrowsingtest-419796688.us-east-1.elb.amazonaws.com/collector', function(d){CliqzUtils.log(d.response);c.send(d.response)}, data, function(err){CliqzUtils.log(err)});
    	}
    	else if(c.metadata.message == "proxy"){
    		var tuM = new tuMessageContext(data);
    		var _data = JSON.parse(data);
    		if(tuM.status){
    			// Based on the dmC get the proxy validators from source map.
    			var routeHash = parseInt(_data["dmC"], 2);
    			var modRoute = routeHash % 40;
				var proxyIDs = CliqzSecureMessage.routeTable[modRoute][1];

				// From the list of proxy validators create validation funcs.
				var validateFuncs = [];
				for(var i=0;i<proxyIDs.length;i++){
					validateFuncs.push(sendMessage(proxyIDs[i], data));
				}

	    		Promise.all(validateFuncs).then(function(values) {
	 				CliqzUtils.log(CliqzSecureMessage.resp,"RESP");
					var upstreamPayload = {};
					// upstreamPayload["proxySignatures"] = json.encode(proxySignatures)
					upstreamPayload["mP"] = _data.mP
					upstreamPayload["dmC"] = _data.dmC
					upstreamPayload["bsmP"] = _data.bs1
					upstreamPayload["bsmPdmC"] = _data.bs3
					upstreamPayload["proxySignatures"] = CliqzSecureMessage.resp;
					upstreamPayload["proxySource"] = CliqzSecureMessage.peerID;
					CliqzUtils.httpPost('http://securebrowsingtest-419796688.us-east-1.elb.amazonaws.com/collector', function(d){CliqzUtils.log(d.response);}, JSON.stringify(upstreamPayload), function(err){CliqzUtils.log(err)});
					//c.send("{'success':true}")
				})
				.catch(function(err){
					CliqzUtils.log("Could not verify","proxy-rejected");
					c.send("Too many requests");
				});
    		}
	    	else{
	    		CliqzUtils.log("Sorry temporalUniquness failed","Status");
	    		c.send("Too many requests");
	    	}

    	}
    	else{
    		CliqzUtils.log(data,"Message recieved");
    	}
    });
...
function sendMessage(requestedPeer, data){
		// CliqzUtils.setTimeout(function(){pp.send(data)},1000)
		var  promise = new Promise(function(resolve, reject){
			try{
				var pConn = CliqzSecureMessage.peer.connect(requestedPeer, {
			        reliable: true,
				...
				pConn.on("open",function(){
					...
					pConn.on("data", function(d){
						CliqzUtils.log(d,"In the functions");
