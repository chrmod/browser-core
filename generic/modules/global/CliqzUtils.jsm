...
//  RESULTS_PROVIDER:               'http://mixer-beta.clyqz.com/api/v1/results?q=',
  ...
  getNoResults: function() {
    ...
    createLazyResourceLoader: function(args) {
      ...
      if (CliqzUtils._resource_loader.interval == null) {
    ...
    _resource_loader: {
      ...
      fetcher: function() {
        ...
        let ind = CliqzUtils._resource_loader.queue.findIndex(function(c) {
          return c.lastPull + c.updateFreq < now && c.retries < 3;
        });
        if (ind >= 0) {
          let load_conf = CliqzUtils._resource_loader.queue[ind];
          CliqzUtils._resource_loader.fetch(load_conf);
          CliqzUtils._resource_loader.queue.push(CliqzUtils._resource_loader.queue.splice(ind, 1)[0]);
        }
      },
